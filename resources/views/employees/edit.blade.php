...
@section('title', 'Edit Employee - Office Management System')
...
$(document).ready(function() {
    console.log('Location selection script loaded for edit form');
    ...
    // Get existing location data from employee
    var existingLocation = @json($employee->location ?? []);
    ...
    function loadCountries() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var selectedCountry = existingLocation.country || '';
                
                data.forEach(function(country) {
                    if (country && typeof country === 'string') {
                        var selected = (country === selectedCountry) ? 'selected' : '';
                        options += '<option value="' + escapeHtml(country) + '" ' + selected + '>' + escapeHtml(country) + '</option>';
                    }
                });
    ...
    function loadStates(country) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var selectedState = existingLocation.state || '';
                
                data.forEach(function(state) {
                    if (state && typeof state === 'string') {
                        var selected = (state === selectedState) ? 'selected' : '';
                        options += '<option value="' + escapeHtml(state) + '" ' + selected + '>' + escapeHtml(state) + '</option>';
                    }
                });
                
                $('#state').html(options).prop('disabled', false);
                
                // If we have a pre-selected state, load its cities
                if (selectedState && data.includes(selectedState)) {
    ...
    function loadCities(country, state) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var selectedCity = existingLocation.city || '';
                
                data.forEach(function(city) {
                    if (city && typeof city === 'string') {
                        var selected = (city === selectedCity) ? 'selected' : '';
                        options += '<option value="' + escapeHtml(city) + '" ' + selected + '>' + escapeHtml(city) + '</option>';
                    }
                });
